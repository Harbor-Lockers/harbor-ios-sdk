...
typedef SWIFT_ENUM(NSInteger, ErrorDomain, open) {
  ErrorDomainSDK = 1,
  ErrorDomainAPI = 2,
  ErrorDomainFirmwareBlink = 3,
  ErrorDomainFirmwareSession = 4,
};

@class Tower;

SWIFT_PROTOCOL("_TtP16HarborLockersSDK24HarborConnectionDelegate_")
@protocol HarborConnectionDelegate
- (void)harborDidDisconnectTower:(Tower * _Nullable)tower;
@end

@class NSError;

SWIFT_CLASS("_TtC16HarborLockersSDK11HarborError")
@interface HarborError : NSObject
+ (BOOL)isSDKError:(NSError * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
+ (BOOL)isAPIError:(NSError * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
+ (BOOL)isFirmwareError:(NSError * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
+ (BOOL)isAuthError:(NSError * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
+ (BOOL)isPermissionsError:(NSError * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
+ (BOOL)isCommunicationError:(NSError * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
+ (BOOL)isSessionError:(NSError * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
+ (BOOL)isHTTPError:(NSError * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
+ (BOOL)isCancelled:(NSError * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
+ (BOOL)isBluetoothError:(NSError * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
+ (BOOL)isNetworkError:(NSError * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_CLASS("_TtC16HarborLockersSDK18HarborErrorFactory")
@interface HarborErrorFactory : NSObject
+ (NSError * _Nonnull)error:(enum SDKErrorCode)code SWIFT_WARN_UNUSED_RESULT;
+ (NSError * _Nonnull)errorWithDomain:(enum ErrorDomain)domain code:(NSInteger)code description:(NSString * _Nonnull)description SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
@property (nonatomic, strong) id <HarborConnectionDelegate> _Nullable connectionDelegate;
...
- (void)downloadTowerConfigurationWithCompletion:(void (^ _Nonnull)(BOOL, NSArray<NSString *> * _Nullable))completion;
- (void)startTowerDiscovery;
- (void)startTowerDiscoveryWithAllowDuplicateScan:(BOOL)allowDuplicateScan;
...
/// \param duration The duration while the requested session packet will be valid to establish a session.
///
/// \param automaticSyncEnabled If automatic sync will be enabled into the created session
///
/// \param sessionPermissions Session permissions to request for.
///
/// \param completionHandler Success if the session was granted successfully.
///
- (void)establishSessionWithTowerId:(NSData * _Nullable)towerId duration:(NSInteger)duration automaticSyncEnabled:(BOOL)automaticSyncEnabled sessionPermissions:(enum SessionPermission)sessionPermissions completionHandler:(void (^ _Nonnull)(BOOL, NSError * _Nullable))completionHandler;
...
/// \param cbor The CBOR packet to send
///
/// \param completionHandler A completion handler block
///
/// \param response Dictionary with the elements in the packet response
///
/// \param error An error returned by the device.
///
- (void)sendRawPacketWithCbor:(NSData * _Nonnull)cbor completionHandler:(NSTimeInterval (^ _Nonnull)(NSDictionary<NSString *, id> * _Nullable, NSError * _Nullable))completionHandler;
@end
...
typedef SWIFT_ENUM(NSInteger, SDKErrorCode, open) {
  SDKErrorCodeUnknown = 0,
  SDKErrorCodePacketNotEncoded = 100,
  SDKErrorCodePacketNotDecoded = 101,
  SDKErrorCodeNotConnectedWithTower = 102,
  SDKErrorCodeResponseTimeout = 103,
  SDKErrorCodeCanceled = 104,
  SDKErrorCodeBluetoothOff = 105,
  SDKErrorCodeDiscoveryFailed = 106,
  SDKErrorCodeDiscoveryCharacteristicFailed = 107,
  SDKErrorCodePeripheralConnectionFailed = 108,
